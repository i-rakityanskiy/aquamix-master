...
interface WaterMixModeProps {
  onToggleMode: () => void;
}

const STORAGE_KEY = 'aquamix_preferences';

const WaterMixMode: React.FC<WaterMixModeProps> = ({ onToggleMode }) => {
  // --- State ---
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4 md:p-8 bg-aqua-mix">
      {/* Header */}
      <header className="w-full max-w-md flex justify-between items-center mb-4 z-10">
        <div className="text-left">
          ...
          <p className="text-slate-500 mt-2 text-sm font-medium">
            Precision Temperature Blending
          </p>
        </div>
        ...
        <button
          ...
          className="p-3 rounded-2xl shadow-lg transition-all active:scale-90 border bg-blue-50 border-blue-100 text-blue-600"
          aria-label="Switch to Brew Mode"
        >
          <LeafIcon />
        </button>
      </header>
